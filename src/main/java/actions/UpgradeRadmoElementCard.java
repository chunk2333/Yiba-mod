...
public class UpgradeRadmoElementCard extends AbstractGameAction {

    private DamageInfo info;

    private AbstractCard theCard = null;

    public UpgradeRadmoElementCard(AbstractCreature target, DamageInfo info) {
        this.info = info;
        ...
        if (this.duration == Settings.ACTION_DUR_MED &&
            ...
            if ((this.target.isDying || this.target.currentHealth <= 0) && !this.target.halfDead &&
                ...
                for (AbstractCard c : AbstractDungeon.player.masterDeck.group) {
                    if (c.canUpgrade() && c.hasTag(YibaMod.ELEMENT))
                        possibleCards.add(c);
                }
